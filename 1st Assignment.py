...
def corpus_statistics(corpus, dv_corpus):
    ...
    questions = ["Number of tokens and types:", "Average token length:", "Longest tokens:", 
                 "Numnber of hapaxes and its probability:", "10th most frequent types (% total tokens):", 
                 "Hapaxes for each partition:", "Percentage of hapaxes per each partition:",
                 "Unique bigrams and their percentage:"]
    
    statistics = [(corpus_length(corpus), number_types(corpus)), average_length(corpus), longest_token(corpus), 
                  (hapaxes(corpus), percentage(hapaxes(corpus), corpus_length(corpus))), 
                  (most_frequent(corpus), percentage_common_types(corpus)), hapaxes_parts(dv_corpus),
                  percentage_hapaxes(dv_corpus, corpus), (n_grams(corpus, 2), n_grams(corpus, 3))]

    return [print(questions[i], statistics[i], '\n') for i in range(len(statistics))]
